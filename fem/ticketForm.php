...
if (isset($_POST['submitTicket'])) {
    ...
    if (!empty($r_personnels && $requestor && $ticket_category)) {
        if (isset($_POST['on_the_spot'])) {
            ...
            $sql = mysqli_query($con, "INSERT INTO request (date_filled, status2, requestor, requestorUsername, email, department, request_type, request_to, request_category, request_details, assignedPersonnel, assignedPersonnelName,assistantsId, assistanNames, ticket_category, ticket_filer)
             VALUES ('$datenow', '$status', '$requestor','$requestorIdnumber', '$requestorEmail', '$requestorDepartment', 'Technical Support', 'fem', '$ticket_category','$detailsOfRequest', '$r_personnels', '$r_personnelsName','$r_assistants','$r_assistantsName', '$ticket_category', '$user_name')");
        }
            ...
            try {
                ...
                if ($onthespot_ticket == '1') {
                    ...
                    foreach ($fem_leader as $item) {
                        $mail2->addAddress($item['email']);  // fem head / leader
                    }
                    ...
                    $mail->clearCCs();
                    foreach ($fem_leader as $item) {
                        $mail->addAddress($item['email']);  // fem head / leader
    ...
    <div class=" ml-72 flex mt-16  left-10 right-5  flex-col  px-14 sm:px-8  pt-6 pb-14 z-50 ">
        ...
        <form class="px-20" method="POST" accept-charset="utf-8" enctype="multipart/form-data" onsubmit="$('#loading-message').css('display', 'flex'); $('#loading-message').show();">
            ...
            <div class=" p-2 border border-inherit rounded-md ">
                ...
                <div class="grid md:grid-cols-1 md:gap-x-6 gap-y-3">
                    <div class="relative z-0 w-full  group">
                        ...
                        <select id="r_categories" name="r_categories" class="js-example-basic-single bg-gray-50 border border-gray-300 text-gray-900 text-xl rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            ...
                            $sql1 = "Select * FROM `femcategories` WHERE req_type = 'TS' ORDER BY hours ASC ";
                            $result = mysqli_query($con, $sql1);
                            while ($list = mysqli_fetch_assoc($result)) {
                                $c_name = $list["c_name"];
                                $hours = $list["hours"];
                ...
                <div class="grid md:grid-cols-2 md:gap-x-6 gap-y-3 mb-10">
                    <div class="relative z-0 w-full  group">
  ...
  tags: true
    ...
    $('#r_assistantsName').val(selectedPersonnels.join(', '));
            ...
            $("#r_personnels option").each(function() {
                ...
                if (assignedSection != 'fem' && assignedSection != "admin") {
                    $(this).hide();
                } else {
                    $(this).show();
                    if (pending >= 5) {
